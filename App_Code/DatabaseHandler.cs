...
public class DatabaseHandler
{
    ...
    //This method returns all table data needed to populate a datagrid
    public DataSet getTableDataAs(string[,] columns)
    {
        string command = "Select ";
        int length = columns.GetLength(1);
        for (int x = 0; x < length; ++x)
        {
            command += "[" +columns[0,x] +"] AS ["+ columns[1,x] + "] ,";
        }
        command = command.Substring(0, command.Length - 1);
        command += " From " + table;
    ...
    public void updateTableData(string [] rowData, string[] columnData, int r)
    {
        command = "UPDATE " + table + " SET ";
        for(int x = 0; x < columnData.Length; ++x)
        {
            command += columnData[x] + " = " + rowData[x] + " , ";
        }
        command = command.Substring(0, command.Length - 2);
        command += " WHERE " + tableColumns[row] +" = "+ rowData[r];
        sConn.Open();
        sCmd = sConn.CreateCommand();
        sCmd = new SqlCommand(command, sConn);
        sCmd.ExecuteNonQuery();
